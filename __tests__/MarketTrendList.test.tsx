...
import MarketTrendList from '../src/components/home/MarketTrendList';
import { Trend } from '../src/types/trends/res/index.type';
...
describe('MarketTrendList', () => {
    it('should render MarketPriceTile components for each trend', () => {
        const { getByText, getAllByTestId } = render(<MarketTrendList trends={mockTrends} />);

        mockTrends.forEach(trend => {
            expect(getByText(trend.name)).toBeTruthy();
        });

        const tileElements = getAllByTestId('market-price-tile');
        expect(tileElements).toHaveLength(mockTrends.length);
    });
});
